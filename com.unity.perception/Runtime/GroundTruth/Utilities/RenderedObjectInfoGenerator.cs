...
namespace UnityEngine.Perception.GroundTruth.Utilities
{
    ...
    static class RenderedObjectInfoGenerator
    {
        ...
        /// InstanceSegmentationRawData should be the raw data from a texture filled by <see cref="InstanceSegmentationLabeler"/>
        ...
        /// <param name="computeInstanceIds"></param>
        public static void Compute(NativeArray<Color32> instanceSegmentationRawData, int stride,
            BoundingBoxOrigin boundingBoxOrigin, out NativeArray<RenderedObjectInfo> renderedObjectInfos,
            Allocator allocator, bool computeInstanceIds = true)
        {
            ...
            using (s_LabelMerge.Auto())
            {
                ...
                for (var i = 0; i < keyValueArrays.Keys.Length; i++)
                {
                    ...
                    uint instanceId = 0;
                    if (computeInstanceIds && !InstanceIdToColorMapping.TryGetInstanceIdFromColor(color, out instanceId))
                        Debug.LogError($"Could not generate instance ID from the color {color}.");
